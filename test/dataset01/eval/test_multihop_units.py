...
def test_anchor_extraction():
    ...
    entities = [a.text for a in anchors if a.kind == "entity" and a.source == "question"]
    assert entities and all(a.confidence >= MIN_ANCHOR_CONFIDENCE for a in anchors if a.source == "question")
    assert all(a.kind == "entity" for a in anchors)

    # 功能词不成锚点
    assert "the" not in [p.casefold() for p in extract_english_phrases("The film was directed by")]
...
def test_hop_queries():
    ...
    q = "Who is Edward Watson, Viscount Sondes's paternal grandfather?"
    first_hop = [{"document_id": "d1", "text": "Edward Watson was the son of John Watson."}]
...
def test_merge_coverage_and_dedup():
    ...
    hop0 = HopResult(hop=0, query="q0", docs=[
    ...
    hop1 = HopResult(hop=1, query="q1", docs=[
...
def test_navigation_gating():
    ...
    # 成员文档重合也算有效
    member_page = {
        "title": "Cluster p3", "entities": ["LongBench"], "keywords": [],
        "documents": ["m1"],
    }
    effective2, _ = gate_navigation("unrelated question text", [member_page],
                                    retrieved_docs=[{"document_id": "m1"}])
    assert len(effective2) == 1 and effective2[0]["navigation_gate"]["member_overlap"] is True
...
def test_passage_title_parser():
    ...
    assert by_idx[1]["passage_title"] == "Waldrada of Lotharingia" and by_idx[1]["is_title_line"]
    assert by_idx[2]["passage_title"] == "Waldrada of Lotharingia" and by_idx[2]["title_source"] == "parsed"
