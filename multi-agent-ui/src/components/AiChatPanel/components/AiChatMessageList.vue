<template>
  <el-scrollbar
    :ref="setScrollbarRef"
    class="afc-scrollbar"
    wrap-class="afc-scroll-wrap"
    @scroll="hideSourceTooltip"
  >
        ...
        >
          <div class="afc-msg__ava" :class="`afc-msg__ava--${msg.role}`">
            <img
              ...
              :src="userAvatar"
              alt="用户头像"
              class="afc-msg__ava-img"
            />
            <el-icon v-else :size="12">
              <Headset v-if="msg.role === 'assistant'" />
              <User v-else />
            </el-icon>
          </div>
          <div class="afc-msg__body">
            <div v-if="msg.thinking" class="afc-thinking-card">
              <div class="afc-thinking-card__hd" @click="emit('toggle-thinking', msg.id)">
                ...
                <span>深度思考过程</span>
                <el-icon
                  class="afc-thinking-card__arrow"
                  :class="{ 'is-open': isThinkingExpanded(msg.id) }"
                  :size="11"
                >
                  <ArrowDown />
                </el-icon>
  ...
  <el-tooltip
    v-if="sourceTooltipVirtualRef"
    virtual-triggering
    :virtual-ref="sourceTooltipVirtualRef"
    :visible="sourceTooltipVisible"
    placement="top"
    popper-class="afc-source-tooltip"
    :popper-options="sourceTooltipPopperOptions"
  >
    <template #content>
      <MdPreview
        :modelValue="sourceTooltipMarkdown"
        class="afc-md-preview afc-source-tooltip__preview"
      />
    </template>
  </el-tooltip>
</template>
