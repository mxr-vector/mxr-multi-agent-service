<template>
    ...
    <footer class="afc-footer">
        <el-select v-if="reasoningOptions.length" :model-value="reasoningEffort" placeholder="思考强度" size="small"
            class="afc-effort-select" @update:model-value="emit('update:reasoningEffort', $event || null)">
            <template #prefix>
                <el-icon :size="13">
                    <MagicStick />
                </el-icon>
            </template>
            <el-option v-for="option in reasoningOptions" :key="option.value" :label="option.label"
                :value="option.value" />
        </el-select>
