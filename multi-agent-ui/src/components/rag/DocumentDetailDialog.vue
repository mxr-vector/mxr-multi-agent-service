...
const statusLabel = computed(
    () => STATUS_LABEL[props.document?.status ?? ""] ?? "未同步"
);
...
<template>
    <el-dialog v-model="dialogVisible" title="文档详情" width="620px">
        ...
        <el-descriptions :column="2" size="small" border class="detail-meta">
            ...
            <el-descriptions-item label="创建时间">{{ fmtTime(document?.created_at) }}</el-descriptions-item>
            <el-descriptions-item label="更新时间">{{ fmtTime(document?.updated_at) }}</el-descriptions-item>
        </el-descriptions>
        ...
        <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
            ...
            <el-form-item label="归属文件夹">
                <el-tree-select v-model="form.folder_id" :data="folderTree"
                    :props="{ label: 'name', children: 'children' }" node-key="id" value-key="id" check-strictly
                    clearable placeholder="根目录" :render-after-expand="false" style="width: 100%" />
            </el-form-item>
