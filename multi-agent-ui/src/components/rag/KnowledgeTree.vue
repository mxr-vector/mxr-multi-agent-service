...
import { listFolders, type Folder } from "@/api/rag/folders";
...
async function fetchFolders(kbId: string): Promise<Folder[]> {
  ...
  const res = await listFolders({ knowledge_base_id: kbId, page: 1, size: 200 });
...
async function loadNode(node: Node, resolve: (data: TreeNodeData[]) => void) {
  if (node.level === 0) {
    const res = await listKnowledgeBases({ page: 1, size: 200 });
