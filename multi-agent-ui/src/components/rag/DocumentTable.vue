...
// 新建文件夹时默认以当前选中文件夹为上级
const folderDefaultParentId = computed(() => (editingFolder.value ? null : selectedFolderId.value));
...
<template>
  <section class="doc-page list-page">
      ...
      <aside class="side">
        ...
        <div class="side-card">
          ...
          <KnowledgeTree
            ...
            :dept-ids="deptFilterIds"
            @select="onTreeSelect"
            @folders-loaded="onFoldersLoaded"
            @cleared="onTreeCleared"
          />
        </div>
      </aside>
    ...
    <DocumentDetailDialog
      ...
      :document="detailDoc"
      :knowledge-base-name="activeKb?.name ?? ''"
      :folders="folders"
