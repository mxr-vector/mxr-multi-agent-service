...
<template>
    <div>
        <input ref="fileInput" type="file" accept=".pdf,.docx,.md,.markdown,.txt,.xlsx,.xls" hidden
            @change="onChange" />
        <section class="upload-zone"><span class="upload-icon">↑</span>
            <div><strong>选择本地文件上传到当前知识库</strong>
                <p>支持 PDF、DOCX、Markdown、Excel、TXT，上传后自动解析与两级切块</p>
            </div><button type="button" :disabled="uploading" @click="open">选择文件</button>
        </section>
    </div>
</template>
