...
async function removeUser(row: User) {
    const confirmed = await confirmDanger(`确定删除用户「${row.username}」吗？其角色关联将一并清理。`);
    if (!confirmed) return;
...
<template>
    <section class="system-page list-page">
        <div class="user-layout">
            ...
            <ListPageCard class="user-list-card" title="用户管理" :subtitle="`共 ${total} 个用户`" :loading="loading">
                <template #actions>
        ...
        <!-- 新建/编辑 弹窗 -->
        <FormDialog v-model="dialogVisible" :title="editing ? '编辑用户' : '新建用户'" width="560px" :submitting="submitting"
            @submit="submit">
            <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
                ...
                <el-form-item v-if="!editing" label="初始密码" prop="password">
                    <el-input v-model="form.password" type="password" show-password placeholder="服务端加密存储"
                        maxlength="100" />
                </el-form-item>
                ...
                <el-form-item label="所属部门">
                    <el-tree-select v-model="form.dept_id" :data="deptTree"
                        ...
                        placeholder="不选表示未分配" style="width: 100%" />
                </el-form-item>
        ...
        <!-- 重置密码 弹窗 -->
        <FormDialog v-model="pwdDialogVisible" :title="`重置密码：${pwdTarget?.username ?? ''}`" width="440px"
            :submitting="pwdSubmitting" confirm-text="确定" @submit="submitResetPassword">
        ...
        <!-- 分配角色 弹窗 -->
        <FormDialog v-model="roleDialogVisible" :title="`分配角色：${roleTarget?.username ?? ''}`" width="440px"
            :submitting="roleSubmitting" @submit="submitAssignRoles">
            <div v-loading="roleLoading">
                ...
                <p v-if="!roleLoading && !roleOptions.length" class="empty-tip">暂无可分配角色，请先创建角色</p>
            </div>
        </FormDialog>
    </section>
</template>
