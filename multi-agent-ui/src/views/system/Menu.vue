        ...
        <section class="content-card list-panel" v-loading="loading" element-loading-text="加载中…">
            ...
            <el-table class="list-scroll" :data="treeData" row-key="id" default-expand-all
                ...
                <el-table-column label="操作" width="170" fixed="right">
                    <template #default="{ row }">
                        <el-button v-if="row.menu_type !== 'button'" link type="primary" size="small"
                            @click="openCreate(row)">新增下级</el-button>
            ...
            <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
                ...
                <el-form-item label="上级菜单">
                    <el-tree-select v-model="form.parent_id" :data="parentOptions"
                        :props="{ label: 'label', children: 'children' }" node-key="id" check-strictly
                        placeholder="不选表示顶级节点" clearable style="width: 100%" />
                </el-form-item>
                <el-form-item label="显示名称" prop="label">
                    <el-input v-model="form.label" placeholder="如：用户管理" maxlength="100" />
                </el-form-item>
                <el-form-item v-if="isMenu" label="路由名称">
                    <el-input v-model="form.name" placeholder="如：SystemUser（前端路由 name）" maxlength="100" />
                </el-form-item>
