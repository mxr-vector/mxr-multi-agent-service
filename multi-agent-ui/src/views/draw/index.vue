...
async function removeSession(session: DrawSessionVO) {
    await ElMessageBox.confirm(`确定删除会话「${session.title}」？将同时删除其消息与图表版本。`, "删除会话", {
        type: "warning",
    });
...
async function afterDone(versionId: string | null) {
    ...
    versions.value = verRes.data ?? [];
    if (versionId) {
        currentVersion.value = versions.value.find((v) => v.id === versionId) ?? versions.value.at(-1) ?? null;
...
async function handleEditorSave(payload: { parent: DrawVersionVO; drawio_xml: string; preview: Blob | null }) {
    ...
    try {
        ...
        versions.value = verRes.data ?? [];
        currentVersion.value = versions.value.find((v) => v.id === res.data?.id) ?? versions.value.at(-1) ?? null;
    } finally {
...
<template>
    <section class="draw-page">
        <div class="draw-layout">
            ...
            <aside class="chat-panel content-card">
                ...
                <div class="message-list">
                    ...
                    <div v-for="message in messages" :key="message.id" class="message" :class="message.role">
                        <img v-if="message.image_file" :src="`${FILE_BASE}/${message.image_file}`" class="message-image"
                            alt="上传图片" />
                        <div v-if="message.content" class="message-bubble">{{ message.content }}</div>
                        <div v-else-if="message.role === 'assistant' && message.status === 'generating'"
                ...
                <div class="composer">
                    ...
                    <div class="composer-actions">
                        ...
                        <div class="composer-buttons">
                            ...
                            <el-button type="primary" size="small" :loading="isLoading" @click="send">发送</el-button>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <div class="canvas-panel content-card">
                <div class="canvas-toolbar">
                    ...
                    <el-button type="primary" size="small" :disabled="!currentVersion || isLoading"
                ...
                <div class="canvas-body">
                    ...
                    <MermaidPreview v-else-if="canvasMermaid" :source="canvasMermaid" :streaming="isLoading"
                        :can-regenerate="!isLoading" @regenerate="regenerate" />
                ...
                <div v-if="versions.length" class="version-bar">
                    ...
                    <div class="version-list">
                        <button v-for="(version, index) in versions" :key="version.id" type="button"
                            class="version-chip"
                            :class="{ active: version.id === currentVersion?.id, user: version.source_type === 'user' }"
        ...
        <DrawioEditorDialog v-model:visible="editorVisible" :version="editorVersion" :saving="editorSaving"
