...
模型名 / 认证参数由配置快照 CFG.rewrite 决定（来源 sys_model_config 的 rewrite
角色行）。实例按配置指纹缓存复用（与 model/chat/factory.py 同机制）：检索热路径
每次工具调用都会取用，复用可跨请求 keep-alive，省去每请求 TCP/TLS 握手。
"""
...
@cache
def _build_compression_model(
    model_name: str, api_url: str, api_key: str, temperature: float
) -> ChatOpenAI:
    """按配置指纹构造 rewrite/compression model 实例（指纹相同即复用）。"""
    return ChatOpenAI(
        model=model_name,
        base_url=api_url,
        api_key=api_key,
...
def build_compression_model(temperature: float = 0) -> ChatOpenAI:
    ...
    """
    rewrite = CFG.rewrite
    return _build_compression_model(
        model_name=rewrite.model_name,
        api_url=rewrite.api_url,
        api_key=rewrite.api_key,
        temperature=temperature,
    )
