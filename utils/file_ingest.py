...
# 用户可选切块策略：auto 有结构则章节切、否则回退字符切；char 纯字符；structure 强制章节
CHUNK_STRATEGIES = ("auto", "char", "structure")
...
def validate_chunk_strategy(strategy: str, doc_type: str) -> None:
    """
    校验策略取值与格式组合：非法取值、structure 搭配无结构格式均抛业务异常。
    auto 对无结构格式静默回退字符切分，不在此拦截。
    """
    if strategy not in CHUNK_STRATEGIES:
        bad_except(f"不支持的分块策略: {strategy}（可选 auto/char/structure）")
...
def ingest_file(
    ...
    strategy 为用户选择的切块策略（auto/char/structure，见 CHUNK_STRATEGIES）：
    - char：全文直接字符切分，与历史行为一致，chapter_title 恒为 None；
    - structure：按章节/标题先切节，节内再字符切分，块携带 chapter_title；
      仅支持 markdown/docx/excel，其余类型抛业务异常；
    - auto：有结构格式走 structure，其余静默走 char。
    ...
    validate_chunk_strategy(strategy, doc_type)
    ...
    # 归一化生效策略：auto 在此落定；char 或无结构格式统一退化为覆盖全文的单节，
    # 使 char 路径与历史字符切分逐字节一致
    use_structure = strategy != "char" and doc_type in _STRUCTURE_DOC_TYPES
