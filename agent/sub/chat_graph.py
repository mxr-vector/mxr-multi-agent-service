...
- 父图负责多轮状态管理（checkpointer 恢复 messages 历史）、condense 问题改写、
  流式生成答案与结构化 sources；
- RAG 检索子图（agent.sub.rag_graph）作为纯检索管线在 rag_retrieve 节点内
...
- rag_retrieve：节点内 `await rag_graph.ainvoke(...)`，输入 standalone 问题
...
from agent.sub.rag_graph import graph as rag_graph
...
condense_model = build_compression_model()

# 编译后的父图单例（首次获取时装配 checkpointer 后编译）
_chat_graph = None
...
async def _load_fallback_history(session_id_hex: str | None, max_messages: int) -> str:
...
# ---------- 节点 ----------
async def condense_node(state: ChatState, config: RunnableConfig):
    ...
    history = _format_history(history_messages, max_messages)

    if not history:
        # checkpoint 无历史（新会话 / TTL 已清理 / 存量会话）：回落业务表
        thread_id = (config.get("configurable") or {}).get("thread_id")
        history = await _load_fallback_history(thread_id, max_messages)
...
async def rag_retrieve_node(state: ChatState):
...
async def respond_node(state: ChatState):
    ...
    history = _format_history(state["messages"][:-1], ENV.chat_history_max_messages)
...
def build_chat_graph(checkpointer):
    ...
    workflow.add_node(ChatNode.CONDENSE.value, condense_node)
    workflow.add_node(ChatNode.RAG_RETRIEVE.value, rag_retrieve_node)
    workflow.add_node(ChatNode.RESPOND.value, respond_node)
...
def get_chat_graph():
    ...
    if _chat_graph is None:
        ...
        _chat_graph = build_chat_graph(get_checkpointer())
    return _chat_graph


def reset_chat_graph() -> None:
    """重置图单例（关停时由 lifespan 调用，避免持有已关闭池的引用）。"""
    global _chat_graph
    _chat_graph = None
