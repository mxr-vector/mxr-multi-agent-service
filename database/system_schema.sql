...
-- ============================================================
-- 10. 菜单种子数据
--     "系统管理" dir 节点 + 六个 menu 子节点;
--     id 用固定 uuidv7 形态字面量, 配合 ON CONFLICT DO NOTHING 保证可重复执行;
--     component 为前端组件键, 对应 multi-agent-ui router/index.ts::viewModules
-- ============================================================
INSERT INTO sys.sys_menu (id, parent_id, menu_type, name, path, component, label, icon, sort_order)
VALUES
    ('01900000-0000-7000-8000-000000000001', NULL, 'dir', 'system', '/system', NULL, '系统管理', 'settings', 90)
ON CONFLICT (id) DO NOTHING;

INSERT INTO sys.sys_menu (id, parent_id, menu_type, name, path, component, label, icon, sort_order)
VALUES
    ('01900000-0000-7000-8000-000000000002', '01900000-0000-7000-8000-000000000001', 'menu', 'system-user',   '/system/user',   'system-user',   '用户管理', 'user',     1),
    ('01900000-0000-7000-8000-000000000003', '01900000-0000-7000-8000-000000000001', 'menu', 'system-role',   '/system/role',   'system-role',   '角色管理', 'role',     2),
    ('01900000-0000-7000-8000-000000000004', '01900000-0000-7000-8000-000000000001', 'menu', 'system-dept',   '/system/dept',   'system-dept',   '部门管理', 'dept',     3),
    ('01900000-0000-7000-8000-000000000005', '01900000-0000-7000-8000-000000000001', 'menu', 'system-menu',   '/system/menu',   'system-menu',   '菜单管理', 'menu',     4),
    ('01900000-0000-7000-8000-000000000006', '01900000-0000-7000-8000-000000000001', 'menu', 'system-dict',   '/system/dict',   'system-dict',   '字典管理', 'dict',     5),
...
--     依赖 username 全局唯一约束, ON CONFLICT DO NOTHING 保证可重复执行;
--     生产环境首次登录后须立即改密(走既有重置密码接口)
-- ============================================================
INSERT INTO sys.sys_user (id, username, password, nickname, status)
VALUES
    ('01900000-0000-7000-8000-0000000000a1', 'admin',
     '$2b$12$ZDbTEc7fpaM5878wuIBBEeqj0T32LYtLnsCPWRggRtorRlzRdQiuu',
     '管理员', 'active')
